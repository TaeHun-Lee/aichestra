...
export function renderDashboardReadModels(data: DashboardReadModels): string {
  ...
  const vaultLiveSummary = asObject(data.vaultIntegration.liveSummary);
  const vaultLiveReadiness = asObject(data.vaultIntegration.liveReadiness);
  const vaultLiveRunRecord = asObject(data.vaultIntegration.liveRunRecord);
  const vaultLiveChecks = asObjects(data.vaultIntegration.liveChecks);
  const policyShadowEvaluatorStatus = asObject(data.policyShadow.evaluatorStatus);
  const policyShadowEvaluatorSummary = asObject(data.policyShadow.evaluatorSummary);
  const policyShadowEvaluatorReport = asObject(data.policyShadow.evaluatorMockReport);
  ...
  <main class="shell">
    ...
    <section class="grid">
      ...
      <aside class="panel">
        ...
        <h2>Durable Collaboration Stores</h2>
        <div class="list">
          <div class="item"><strong>Status</strong><span>${html(data.collaborationStores.summary.status, "v1_implemented")} / provider ${html(data.collaborationStores.summary.providerKind, "in_memory")} / default runtime ${html(data.collaborationStores.summary.defaultRuntime, "in_memory")} / production ready false</span></div>
          <div class="item"><strong>Repository coverage</strong><span>groups ${html(data.collaborationStores.summary.implementedRepositoryGroupCount, "0")}/${html(data.collaborationStores.summary.repositoryGroupCount, "0")} / required records ${html(data.collaborationStores.summary.requiredDurableRecordCount, "0")} / recommended ${html(data.collaborationStores.summary.recommendedDurableRecordCount, "0")}</span></div>
          <div class="item"><strong>Repository groups</strong><span>${html(data.collaborationStores.repositories.map((repository) => `${text(repository.group)}:${text(repository.providerKind)}:${text(repository.implemented)}`), "none")}</span></div>
          <div class="item"><strong>Schema coverage</strong><span>${html(data.collaborationStores.schema.status, "implemented_skeleton")} / tables ${html(data.collaborationStores.schema.tableCount, "0")} / event table ${html(data.collaborationStores.schema.eventTableName, "durable_collaboration_events")}</span></div>
          <div class="item"><strong>Storage gates</strong><span>configured ${data.collaborationStores.summary.durableCollaborationStoreConfigured === true ? "true" : "false"} / optional Postgres ${data.collaborationStores.summary.optionalPostgresSupported === true ? "supported" : "not supported"} / optional DB tests ${data.collaborationStores.summary.optionalPostgresTestsConfigured === true ? "configured" : "skipped"}</span></div>
          <div class="item"><strong>Safety</strong><span>remote Git ${data.collaborationStores.safety.remoteGitOperationsExecuted === true ? "executed" : "not executed"} / workspace mutation ${data.collaborationStores.safety.workspaceMutationExecuted === true ? "executed" : "not executed"} / external calls ${data.collaborationStores.safety.externalCallsExecuted === true ? "executed" : "not executed"}</span></div>
          <div class="item"><strong>No-secret/no-env status</strong><span>no secrets ${data.collaborationStores.noSecretStatus.noSecretsExposed === true ? "true" : "false"} / env values exposed ${data.collaborationStores.noSecretStatus.envValuesExposed === true ? "true" : "false"} / DB URL exposed ${data.collaborationStores.noSecretStatus.databaseUrlExposed === true ? "true" : "false"} / raw payload storage ${data.collaborationStores.noSecretStatus.rawPayloadStorageAllowed === true ? "allowed" : "forbidden"}</span></div>
        </div>
        ...
        <div class="list">
          ...
          <div class="item"><strong>Live enablement</strong><span>status ${html(vaultLiveSummary.readinessStatus ?? vaultLiveReadiness.status, "skipped")} / manual live validation ${vaultLiveSummary.canRunLiveValidation === true ? "ready" : "not ready"} / run record ${html(vaultLiveRunRecord.status, "skipped")} / production Vault rollout ${vaultLiveSummary.productionVaultRolloutImplemented === true ? "true" : "false"}</span></div>
          ...
          <div class="item"><strong>Live checks</strong><span>${html(vaultLiveChecks.map((check) => `${text(check.checkKind)}:${text(check.status)}:${text(check.severity)}`), "none")}</span></div>
        ...
        <div class="list">
          ...
          <div class="item"><strong>External Observability Export</strong><span>${html(asObject(data.observability.exportReadinessSummary.metadata).status, "v1_implemented_skeleton")} / exporter enabled ${data.observability.exportReadinessSummary.exporterEnabled === true ? "true" : "false"} / external calls ${data.observability.exportReadinessSummary.externalCallsEnabled === true ? "enabled" : "disabled"} / future backends ${html(data.observability.exportReadinessSummary.futureExporterCount, "0")}</span></div>
          <div class="item"><strong>Export safety</strong><span>checks ${html(data.observability.exportReadinessSummary.safetyCheckCount, "0")} / failed ${html(data.observability.exportReadinessSummary.failedSafetyCheckCount, "0")} / raw payload export ${data.observability.exportReadinessSummary.rawPayloadExportAllowed === true ? "true" : "false"} / secret export ${data.observability.exportReadinessSummary.secretExportAllowed === true ? "true" : "false"}</span></div>
          <div class="item"><strong>Exporter configs</strong><span>${html(data.observability.exporterConfigs.map((config) => `${text(config.exporterKind)}:${text(config.status)}`), "none")} / endpoints hidden / auth hidden</span></div>
        ...
        <div class="list">
          ...
          <div class="item"><strong>Evaluator skeleton</strong><span>skeleton ${data.policyShadow.summary.shadowEvaluatorSkeletonImplemented === true ? "implemented" : "not implemented"} / evaluator enabled ${policyShadowEvaluatorStatus.enabled === true ? "true" : "false"} / running ${policyShadowEvaluatorStatus.running === true ? "true" : "false"} / source ${html(policyShadowEvaluatorStatus.sourceOfTruth, "StaticPolicyEngine")} / mock comparison ${policyShadowEvaluatorSummary.mockComparisonSupported === true ? "ready" : "not ready"} / golden cases ${html(policyShadowEvaluatorSummary.goldenCaseCount, "0")}</span></div>
          <div class="item"><strong>Mock report</strong><span>compared ${html(policyShadowEvaluatorSummary.comparedMockResults, "0")} / mismatches ${html(policyShadowEvaluatorSummary.mismatchCount, "0")} / critical ${html(policyShadowEvaluatorSummary.criticalMismatchCount, "0")} / enforcement changed ${policyShadowEvaluatorSummary.enforcementChanged === true ? "true" : "false"} / preview ${html(policyShadowEvaluatorReport.previewCount, "0")}</span></div>
          ...
          <div class="item"><strong>Evaluator mismatch types</strong><span>${html(data.policyShadow.evaluatorMismatchTypes.map((mismatch) => `${text(mismatch.mismatchKind)}:${text(mismatch.severity)}:${text(mismatch.defaultAction)}`), "none")}</span></div>
