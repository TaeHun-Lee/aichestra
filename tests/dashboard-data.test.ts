...
test("dashboard data exposes conflict manager v1 simulation and registry package assumptions", async () => {
  ...
  assert.equal(data.providerInvocation.status, "unavailable");
  assert.equal(data.providerInvocation.error?.code, "local_agent_required");
  assert.equal(data.providerAuditEvents.some((event) => event.eventType === "local_agent_required"), true);
});
