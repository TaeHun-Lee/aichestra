...
`aich session complete <session-id>` is the candidate creation boundary. It inspects the session worktree, commits any dirty tracked or unignored untracked changes on the session branch, records the patch set and Change Manifest, and marks sessions with real diffs as `enqueued`. Changed symbols are captured with MVP diff heuristics from hunk headers and changed declaration lines, then stored as `changed_files.symbols_json` and rendered into the generated manifest draft. If the session branch has no diff from its recorded base commit, the session is marked `noop` and does not enter the merge queue.

By default, `manifest.adapter: generated` stores the deterministic draft with `validation_status: generated_from_diff`. If `manifest.adapter` is `command` or `llm`, completion renders the generated draft, actual diff stat/patch, changed-file evidence, context snapshot hash, configured Change Manifest prompt, and latest session-agent stdout/stderr artifacts into a provider input. The provider must return a full `change_manifest:` YAML document. Aichestra then overwrites trusted identity and evidence fields from local data, records `generated_by_command` or `generated_by_llm`, and refuses completion if the provider manifest is invalid or omits any actual changed file from structured manifest evidence.
