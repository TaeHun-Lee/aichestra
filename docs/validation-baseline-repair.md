...
The build failure was a path mismatch. `scripts/build.mjs` expected `AICHESTRA_BOOTSTRAP.md` at the repository root, but the canonical copy had already been moved under `design_docs/AICHESTRA_BOOTSTRAP.md`.
...
- Updated `scripts/build.mjs` to validate `design_docs/AICHESTRA_BOOTSTRAP.md` instead of requiring a duplicate root copy.
