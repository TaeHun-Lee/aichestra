...
`aich manifest regenerate <session-id>` reruns the configured `manifest.adapter` against the latest recorded patch set, changed-file evidence, existing diff artifacts, current context snapshot hash, configured prompt, and latest session-agent artifacts. The command writes a fresh manifest to the existing manifest artifact path, updates the ledger hash and validation status, refreshes provider-generation artifacts when the adapter is `command` or `llm`, and records `manifest.updated` with `update_kind: regenerated`. It preserves the candidate patch, verified commit, verified tree, and approval state boundary. Already approved, applying, applied, or abandoned sessions cannot have their current manifest regenerated.
