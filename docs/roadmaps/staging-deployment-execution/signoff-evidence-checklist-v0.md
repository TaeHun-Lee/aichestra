...
| Signoff scope separation policy | `docs/roadmaps/staging-deployment-execution/scope-separation-policy-v0.md` | all roles | Defines how reviewed target scope, evidence scope, execution scope, and governance/policy scope are separated so later audit/readiness/request documents do not automatically dirty the approved target. | present |
