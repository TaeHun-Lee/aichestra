...
- Status: **planned_only**
- Evidence: `docs/foundations/security-model.md`, `docs/foundations/auth-rbac-readiness.md`. No real SSO/SCIM, signed artifacts, policy-as-code engine, sandbox, secrets manager, deployment IaC, data residency, audit export.
...
| 19 | Dashboard | Status, PR, registries, conflicts, improvement, integrations | Implemented but **architecturally separated only partially** | `apps/web/src/render.ts`, `apps/web/lib/mock-data.ts`, `apps/web/lib/improvement-demo.ts` | **partial** | Dashboard still constructs its own seeded store and runs workflows at render time; the read-model migration is planned in `docs/features/dashboard/read-model-plan.md` but not done. |
...
| 24 | Production Auth/RBAC | Real SSO/SCIM, request auth, actor resolution | **Not implemented** | `docs/foundations/auth-rbac-readiness.md` enumerates proposed actor/role/permission model; no implementation | **fail-by-design** | Documented Phase 5 plan. |
...
| `pnpm build` | pass | `node scripts/build.mjs` validates 27 required paths (now includes `docs/briefs/AICHESTRA_BOOTSTRAP.md`, all Phase 4 / real-integration / Postgres / LLM Gateway docs, and `infra/migrations/0001_initial_aichestra_schema.sql`), then `node --check` on the three app entry files. |
...
- `docs/audits/2026-05-11-phase-progress-audit.md` reflects the current Phase 4 v1 + LLM Gateway v0 + Real Git Adapter v0 + Persistent DB v1 + Real Integration Foundation v0 state.
- `docs/features/persistent-db/v1.md`, `docs/features/real-git-adapter/v0.md`, `docs/features/llm-gateway/v0.md`, `docs/features/auto-improvement/preparation.md`, `docs/features/auto-improvement/v0.md`, `docs/features/governance/v1.md` are all consistent with implementation.
- `docs/audits/2026-05-12-final-audit-synthesis.md`, `docs/audits/2026-05-12-validation-baseline-repair.md` correctly describe the post-blocker fix (failing dashboard test + build script path).
- `docs/foundations/auth-rbac-readiness.md`, `docs/features/real-git-adapter/audits/v0-readiness.md`, `docs/features/dashboard/read-model-plan.md`, `docs/roadmaps/real-integration-roadmap.md` are forward-looking design notes; they do not claim implementation.

Stale docs:
- `docs/features/auto-improvement/preparation-blocked.md` and `docs/features/auto-improvement/v0-blocked.md` describe the pre-fix state. They are accurate as history but should be archived or annotated as "resolved" so a new contributor doesn't think Phase 4 is still blocked.
- `docs/audits/2026-05-11-phase-1-2-completion-audit.md` says "phase 4 planned_only" — out of date now that Phase 4 v1 exists. Worth refreshing or stamping as a historical snapshot.
...
Missing docs:
- No dedicated `docs/features/local-agent-runner/v0-plan.md` or similar for the recommended next task.
- No OpenAPI / route documentation for the API.
- No dedicated `docs/foundations/architecture.md` overview tying all packages together (a `docs/briefs/AICHESTRA_TECH_STACK_BLUEPRINT.md` exists; a one-page `docs/foundations/architecture.md` already exists but is short — it could be refreshed).
...
|---|---|---|---|
| Real auth/RBAC | missing | API has no authentication. Cannot use beyond a single trusted developer/team. | Phase 5 (planning doc exists in `docs/foundations/auth-rbac-readiness.md`). |
...
2. **Dashboard read-model migration.** Move `apps/web/lib/mock-data.ts` + `improvement-demo.ts` from in-process workflow execution to consuming the API (or a server-side read model). Plan in `docs/features/dashboard/read-model-plan.md`.
...
5. **Refresh historical docs.** Mark `docs/features/auto-improvement/preparation-blocked.md`, `docs/features/auto-improvement/v0-blocked.md`, and `docs/audits/2026-05-11-phase-1-2-completion-audit.md` as historical snapshots so they don't mislead.
...
- The roadmap (`docs/roadmaps/real-integration-roadmap.md`, README "Next Steps" #1) already lists this as the next step.
