...
CLAUDE.md was not present, so AGENTS.md was read directly. README.md and all referenced docs in `docs/` and `design_docs/` were inspected. The doc named `AICHESTRA_BOOTSTRAP.md` lives at `docs/briefs/AICHESTRA_BOOTSTRAP.md`, not at the repo root (significant — see Section 9 and 11).
...
- Confidence: **high**
- Docs vs code: **mostly agree, with one notable mismatch**. The docs (README, `docs/audits/2026-05-11-phase-progress-audit.md`, `docs/audits/2026-05-11-phase-3-completion-gap.md`) describe Phase 1, Phase 2 v1 Conflict Manager, and Phase 3 v3 Registry capabilities that are in fact implemented in code. However the docs report "validation passing" while validation does not pass today: `pnpm test` fails 1/68 tests (dashboard fixture), and `pnpm build` fails because `scripts/build.mjs` requires `AICHESTRA_BOOTSTRAP.md` at the repo root but the file lives under `design_docs/`.
...
- Production-readiness claims: **none overstated in critical places**. README explicitly says "MVP scaffold... mock-first". `docs/audits/2026-05-11-phase-progress-audit.md` correctly states "not production-ready". `Mock RBAC is not production auth` is encoded in `AGENTS.md` rule 51.
...
- Status: **not_started** (planned_only; explicitly **blocked** in `docs/features/auto-improvement/preparation-blocked.md` and `docs/features/auto-improvement/v0-blocked.md`).
- Evidence:
  ...
  - `docs/features/auto-improvement/preparation.md` and `docs/features/auto-improvement/v0.md` are absent. Only the `*-blocked.md` placeholders exist.
...
- Status: **planned_only**
- Evidence: `docs/foundations/security-model.md`, references in `AGENTS.md` (mock RBAC, mock secrets, mock MCP). No real auth, SSO, SCIM, signed artifacts, policy-as-code engine, sandbox, secret broker, deployment IaC.
...
| `pnpm test` (`node --test` on all `*.test.ts`) | **fail** | 68 tests, 67 pass, 1 fail. Failing test: `tests/dashboard-data.test.ts:6` — `data.registryVersionResolution.selected?.version` is `undefined`, expected `"1.0.0"`. Cause is documented in `docs/features/auto-improvement/preparation-blocked.md`: dashboard fixture marks `auth-debugging` approval as `pending`, then resolves `auth-debugging@^1.0.0`; resolver correctly excludes pending entries. |
| `pnpm build` (`node scripts/build.mjs`) | **fail** | `ENOENT: no such file or directory, access '/home/einzlth/Projects/aichestra/AICHESTRA_BOOTSTRAP.md'`. The file lives at `docs/briefs/AICHESTRA_BOOTSTRAP.md`. `scripts/build.mjs:7` lists `AICHESTRA_BOOTSTRAP.md` as a required root path. |
...
These two failures are the actual blockers to advancing to Phase 4 Preparation, exactly as Codex previously documented in `docs/features/auto-improvement/preparation-blocked.md` and `docs/features/auto-improvement/v0-blocked.md`.
...
- `docs/audits/2026-05-11-phase-progress-audit.md` — accurate phase status; one stale "AICHESTRA_BOOTSTRAP.md at root" assumption (now under `design_docs/`).
- `docs/audits/2026-05-11-phase-3-completion-gap.md` — accurate.
- `docs/features/registry/v3-packaging-versioning.md` — accurate (matches `registry/src/index.ts`).
- `docs/features/conflict-manager/v0.md`, `docs/features/conflict-manager/v1.md` — accurate.
- `docs/features/auto-improvement/preparation-blocked.md`, `docs/features/auto-improvement/v0-blocked.md` — accurate; correctly explain the failing test root cause.

Stale or inconsistent docs:
- `docs/audits/2026-05-11-bootstrap-gap-report.md:53` and `docs/audits/2026-05-11-vertical-slice-review.md:42` say `AICHESTRA_BOOTSTRAP.md` is at the repository root, but the file is now under `design_docs/`. `scripts/build.mjs:7` shares the same stale expectation.
- `docs/briefs/AICHESTRA_CODEX_PHASE3_TO_PHASE4_WORK_ORDERS.md` "Current Assumed Project State" header claims `pnpm test: pass, 54 tests`; the repo now has 68 tests and 1 failing — the work-order pretext is out of date.
- README "Next Steps" #1 says "Execute Work Order 3: Phase 4 Preparation"; this remains blocked per `docs/features/auto-improvement/preparation-blocked.md`.
...
Missing docs:
- `docs/features/auto-improvement/preparation.md` (work-order target) — not written.
- `docs/features/auto-improvement/v0.md` — not written.
- `docs/features/conflict-manager/v0.md` exists but no `docs/features/registry/v0.md` *follow-on for hardening* in a single place — already split across v0/v1/v2/v3 files; acceptable.
- No OpenAPI spec; README #4 lists it as a next step.
- No `docs/audits/2026-05-12-final-completion-audit.codex.md` parallel for cross-comparison (this audit is the Claude version).
...
1. `pnpm test` is failing 1/68: `tests/dashboard-data.test.ts:21` asserts `registryVersionResolution.selected?.version === "1.0.0"`, but the same fixture marks `auth-debugging` approval as `pending`, and the resolver correctly excludes pending entries. Fix per `docs/features/auto-improvement/preparation-blocked.md` (either change fixture or change assertion).
2. `pnpm build` is failing because `scripts/build.mjs:7` requires `AICHESTRA_BOOTSTRAP.md` at the repo root, but the file is at `docs/briefs/AICHESTRA_BOOTSTRAP.md`. Update `requiredPaths` to the new location (and ideally also list `docs/briefs/AICHESTRA_TECH_STACK_BLUEPRINT.md` and the codex work-order docs explicitly).
...
5. Update the stale "bootstrap at root" sentences in `docs/audits/2026-05-11-bootstrap-gap-report.md` and `docs/audits/2026-05-11-vertical-slice-review.md`, and the test-count claim in `docs/briefs/AICHESTRA_CODEX_PHASE3_TO_PHASE4_WORK_ORDERS.md`.
...
- Update `scripts/build.mjs` `requiredPaths` to reference `docs/briefs/AICHESTRA_BOOTSTRAP.md`.
- Rerun `pnpm lint && pnpm typecheck && pnpm test && pnpm build`.
- Then begin Work Order 3 per `docs/briefs/AICHESTRA_CODEX_PHASE3_TO_PHASE4_WORK_ORDERS.md`, scoped strictly to mock-first models + APIs + tests with no auto-mutation path.
