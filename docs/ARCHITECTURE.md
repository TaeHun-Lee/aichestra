...
6. If `manifest.adapter` is `command` or `llm`, Aichestra sends the generated draft, actual diff evidence, context snapshot hash, configured prompt, and latest session-agent artifacts to the manifest generator, then preserves trusted identity/diff evidence fields before storing the provider output. Invalid provider YAML or missing structured changed-file evidence refuses completion before enqueue.
7. The stored manifest records diff-derived file and symbol evidence and remains subject to human/semantic review. `aich manifest show`, `aich manifest edit`, and `aich manifest regenerate` let the operator inspect, refine, or rerun the configured manifest generator before semantic review; edits and regeneration update the artifact hash and record `manifest.updated` without changing the candidate patch or verified tree.
8. A candidate enters the local merge queue and appears in `aich queue` as `enqueued`.
9. Preflight acquires the local `merge-queue` lock, then creates a temporary sandbox from latest main.
10. The candidate is mechanically merged using the same strategy that will be used to apply.
11. Checks run in the sandbox and store stdout/stderr artifacts. The merge attempt also stores a fingerprint of the check policy from `.aichestra/config.yaml`.
12. `aich review` calls the semantic review adapter, then writes a report from the manifest, related applied/queued manifests, diff evidence, mechanical merge result, configured prompt, and sandbox check results. The review row records evidence fingerprints for the manifest, verified candidate, changed files, sandbox checks, and semantic review policy, so approval can reject stale review evidence after any of those inputs change. Required checks form the gate, while optional checks remain review evidence. If the configured check policy changes after preflight, review/approval/apply first require a fresh preflight. If the semantic review adapter, block policy, or prompt changes after review, approval/apply require a fresh review. The default MVP adapter is local and deterministic; provider-backed LLM adapters must preserve the same advisory boundary.
13. If semantic review produces a proposed patch or fix plan, Aichestra stores it as an artifact and requires the user to choose rework or explicit current-tree approval before normal approval can continue.
14. `aich approve` records human approval for the exact verified tree/commit, including the operator identity.
15. If the user decides the verified candidate should not be applied, `aich reject <session-id> --reason TEXT` records a human rejection and blocks that verified attempt. `aich session reopen <session-id>` returns the session to `running` so the session worktree can be revised and completed again.
16. `aich apply` acquires the same local `merge-queue` lock, then fast-forwards main to the approved verified commit only after rechecking main has not moved.
17. A candidate that should not continue can be withdrawn with `aich session abandon <session-id>`, which marks the session `abandoned` and removes it from the queue view without touching main.
18. After apply or abandon, `aich session cleanup <session-id>` or `aich session prune --applied|--inactive` can remove session worktrees, session branches, and related sandbox worktrees. No-op and failed-start sessions can also be cleaned explicitly, or pruned with `aich session prune --inactive`.
...
- Preflight runs configured checks inside the integration sandbox through the `aich-check` runner with explicit `required`, timeout, and environment settings. The normalized check policy is fingerprinted on the merge attempt. Required check failures or timeouts block verification; optional check failures are recorded for review without bypassing the required gate.
...
- Review, approval, and apply require the current check policy fingerprint to match the policy used by preflight; changing check commands or gate settings sends the candidate back to `aich preflight`.
- Approval and apply require the current semantic review policy fingerprint to match the policy used by the latest review; changing adapter settings, risk block levels, or prompt content sends the candidate back to `aich review`.
- Approval and apply require semantic review evidence that matches the current Change Manifest id/hash, verified candidate, changed files, and sandbox check evidence; changing any of those after review sends the candidate back to `aich review`.
