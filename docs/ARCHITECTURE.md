...
11. `aich review` calls the semantic review adapter, then writes a report from the manifest, related applied/queued manifests, diff evidence, mechanical merge result, configured prompt, and sandbox check results. The review row records evidence fingerprints for the manifest, verified candidate, changed files, and sandbox checks, so approval can reject stale review evidence after any of those inputs change. Required checks form the gate, while optional checks remain review evidence. If the configured check policy changes after preflight, review/approval/apply first require a fresh preflight. The default MVP adapter is local and deterministic; provider-backed LLM adapters must preserve the same advisory boundary.
