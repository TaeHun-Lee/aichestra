...
11. `aich review` calls the semantic review adapter, then writes a report from the manifest, related applied/queued manifests, diff evidence, mechanical merge result, configured prompt, and sandbox check results. The review row records the manifest id/hash it reviewed, so approval can reject stale review evidence after a manifest edit. Required checks form the gate, while optional checks remain review evidence. The default MVP adapter is local and deterministic; provider-backed LLM adapters must preserve the same advisory boundary.
...
- Approval requires semantic review evidence that matches the current Change Manifest id/hash; editing manifest intent after review sends the candidate back to `aich review`.
