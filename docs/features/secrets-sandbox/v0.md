...
Real Git Adapter v1 reads the GitHub token only from explicit environment/config gates for controlled branch/PR creation. The token is not stored in `SecretRef`, repos, pull request records, audit events, health responses, dashboard data, runner metadata, or Local Agent metadata. Moving GitHub credentials into a production `SecretManager`/`SecretRef` flow is future work; v1 does not issue a `SecretLease` for GitHub and does not inject secrets into runners or Local Agents.
...
Local Agent Protocol v1 is implemented as a controlled local fixture daemon simulation. Next recommended task: Real Git Adapter v1, if controlled remote Git branch/PR creation should be enabled next, or LLM Gateway v1 if controlled real provider calls should be enabled next.
