...
## Real Git Adapter v1 Integration

Real Git Adapter v1 can resolve GitHub credentials through SecretRef-backed Provider Credentials v1. Preferred config uses `AICHESTRA_GITHUB_TOKEN_SECRET_REF` plus the explicit env provider and env-key allowlist. Legacy `AICHESTRA_GITHUB_TOKEN` remains supported when no SecretRef is configured. The token is not stored in `SecretRef`, repos, pull request records, audit events, health responses, dashboard data, runner metadata, or Local Agent metadata.
...
SecretRef-backed Provider Credentials v1 is implemented for env-backed provider credentials. Next recommended task: Real Git Adapter v2 / webhook planning, or LLM Gateway v2 multi-provider routing.
