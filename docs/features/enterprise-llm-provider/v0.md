...
Cloud/OAuth/WIF/IAM adapters return blocked results. LLM Gateway v1 can use the `openai-api-key` catalog entry as sanitized provider metadata for the controlled `openai_compatible` HTTP path, but ProviderAdapter cloud invocation remains blocked. Local CLI now routes through Local Agent Protocol v1 when a compatible fixture Local Agent, established mock channel, and approved consent exist; otherwise it returns `local_agent_required`, `local_agent_unavailable`, `channel_required`, `awaiting_consent`, `consent_denied`, or `provider_template_incompatible`. PTY returns `disabled_by_policy`.
...
- No real provider API calls through Enterprise ProviderAdapter skeletons. LLM Gateway v1 has a separate controlled OpenAI-compatible HTTP provider path behind explicit gates.
- No real OAuth/device-code/WIF/IAM exchange.
- SecretRef-backed provider credentials are available through `packages/security` for cloud API style credentials, but provider adapter skeletons remain blocked and no production secret manager is implemented.
...
Next recommended task: Real Git Adapter v2 / webhook planning, or LLM Gateway v2 multi-provider routing.
