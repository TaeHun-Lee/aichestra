...
Cloud/OAuth/WIF/IAM adapters return blocked results. Local CLI now routes through Local Agent Protocol v1 when a compatible fixture Local Agent, established mock channel, and approved consent exist; otherwise it returns `local_agent_required`, `local_agent_unavailable`, `channel_required`, `awaiting_consent`, `consent_denied`, or `provider_template_incompatible`. PTY returns `disabled_by_policy`.
...
Local Agent Protocol v1 now coordinates local CLI providers through mock registration, mock channel/handshake, fixture daemon simulation, compatibility checks, consent, sandbox/redaction references, normalized stream events, policy checks, and audit records. Enterprise provider entries may reference secret metadata, but provider adapters still receive no raw credentials and remain blocked by default unless mock Local Agent consent, compatibility, channel, and transport gates pass.
