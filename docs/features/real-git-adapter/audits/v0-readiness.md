...
Status: implemented as a safe v0 boundary in `docs/features/real-git-adapter/v0.md`, with `MockGitProvider`, `LocalGitProvider`, a gated `GitHubGitProvider` skeleton, `GitIntegrationService`, API routes, dashboard visibility, and deterministic tests.
