...
In v0, `packages/adapters/src/git/github-git-provider.ts` was a skeleton only:
...
Update: Real Git Adapter v1 now adds controlled GitHub branch, PR, and PR changed-file operations behind explicit gates. See `docs/features/real-git-adapter/v1.md`. The default runtime remains mock-first, and merge/rebase/force-push/branch deletion are still out of scope.
...
- v0 itself keeps GitHub as a gated skeleton and does not call GitHub.
- Controlled remote branch/PR creation is implemented only in v1 and only behind explicit GitHub gates/allowlists.
...
Real Git Adapter v1 has been implemented. Next recommended tasks are Dashboard API-backed read model migration, or LLM Gateway v1 if controlled real provider calls should be enabled next.
