...
| Future secret backend env vars | Vault/cloud secret backend selection | unset | future | security | future required | yes for backend credentials | status only | status only | yes | future |
...
| Future observability env vars | Log/metric/trace exporters | unset | future | ops | future required | possible | status only | status only | yes | future |

## Production Rules

- `AICHESTRA_STORAGE_PROVIDER=postgres` is required.
- `AICHESTRA_DASHBOARD_DATA_SOURCE=api` is required.
- Mock auth, legacy env credentials, real MCP transport without allowlists, unbounded LLM fallback, local command execution, and vendor CLI credential cache reads must be rejected.
