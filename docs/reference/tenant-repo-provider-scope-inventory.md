...
Status: Tenant/Repo/Provider Scope Model `v1_implemented`; Dashboard/Readiness Tenant Scope Planning `v1_implemented`; Dashboard/Readiness Tenant Scope Implementation `v1_implemented`; Tenant Scope Enforcement `v1_implemented_partial`; Registry Tenant Scope Enforcement `v1_implemented_partial`.
...
| registry_package | `RegistryPackageScope` | sample skill, harness, instruction packages | Registry/Governance policy/audit metadata; Registry Tenant Scope Enforcement v1 decisions for skills, harnesses, instructions, packages, resolver results, approval queues, audit/readiness, and mutation checks; Registry Artifact Trust v1 attribution metadata for package trust decisions | registry package helper plus `registry_scope` and `registry_artifact_trust` policy resources | registry package scope in migrated mutation/governance audit metadata and artifact trust metadata | `/readiness/scopes/registry-packages`, `/registry/scope/*`, `/readiness/registry/scope/summary`, `/registry/artifact-trust/*`, `/readiness/registry/artifact-trust/summary`, dashboard registry scope/trust panels | partial representative metadata enforcement plus mock artifact trust metadata | tenant package catalogs, durable grants, production filters, signed-bundle verification, and real artifact registry | resolver gates unchanged; no production isolation; no real signing/verification |
...
- Registry Tenant Scope Enforcement v1 applies that helper to registry resources and preserves lifecycle, approval, eval, checksum, semver, policy, and governance gates. Registry Artifact Trust v1 uses the same scope vocabulary as attribution metadata only and does not implement production signing or tenant isolation.
