...
- `packages/policy`: provider-neutral Policy-as-code Skeleton v0 models, static/mock policy engine, default restrictive rules, RequestContext-enriched policy subjects, `PolicyResourceScope` helpers, Policy Runtime PoC Golden Test Harness v1 fixtures/harness, Policy Runtime Shadow Evaluator Skeleton v1 disabled/mock evaluators and comparison models, decision audit, DTOs, and policy service boundaries. No live shadow evaluator or candidate runtime is implemented.
...
- `packages/observability`: Observability / Audit Retention v0 common audit envelope, retention/redaction classes, audit sanitizer, source normalization, read-only retention policies, metric snapshots, trace skeletons, no-external-export observability read models, Audit Query Scope Enforcement v1 partial decision/redaction metadata, and External Observability Export v1 disabled/mock/future exporter skeletons, export envelopes, safety checks, readiness summaries, and no-external-call/no-raw-payload guarantees.
- `packages/adapters`: compatibility aggregate for shared adapter contracts and mocks.
- `packages/db`: Postgres-oriented schema, storage provider abstraction, repository factory, in-memory repositories for mock-first runtime/tests, opt-in Postgres repositories for the core durable slice, and Durable Collaboration Stores v1 repository/schema boundaries for branch/session/workspace/worktree/edit/merge/conflict/PR/cleanup metadata.
...
Docker can run the API and web dashboard together:

```bash
docker compose up --build
```

The compose stack exposes the API at `http://localhost:3000` and the web dashboard at `http://localhost:3001`. The web container uses API-backed dashboard data via `AICHESTRA_DASHBOARD_API_BASE_URL=http://api:3000`, so it does not need a host-local API URL. Stop the stack with:

```bash
docker compose down
```

The root compose file keeps Postgres optional. Include it only when testing the gated Postgres path:
...
Durable Collaboration Stores v1 follows the same storage gates. The collaboration-store repository boundary is available in-memory by default, optional Postgres tables/repositories are selected only through the existing Postgres provider, and API/dashboard/health surfaces expose only provider/status/count metadata.
...
Validation also covers Policy Bundle Runtime PoC Planning v0, Policy Runtime PoC Golden Test Harness v1, Policy Runtime Shadow Evaluator Skeleton v1, and OIDC Provider Skeleton Hardening v1 as readiness-only/offline safety surfaces without dynamic policy runtime execution or real identity-provider calls.
...
2. Production Auth Provider Skeleton v1, Tenant Scope Enforcement v1, Dashboard Scope Filtering v1, Audit Query Scope Enforcement v1, and External Observability Export v1 are implemented as disabled/readiness-only, partial representative, or check-only metadata; implement Readiness Endpoint Scope Filtering v1, Tenant Scope Enforcement v2, Audit Query Scope Enforcement v2, or External Observability Export Integration-Test Profile v1 before any production tenant filtering or observability export expansion.
