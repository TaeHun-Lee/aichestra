...
- `packages/deployment-readiness` owns Production Deployment Readiness Planning v0 read-only deployment profile models, readiness check models, production risk models, DTOs, and deterministic seed data. It must not run live infrastructure checks, deploy resources, call external providers, read secrets, or enforce production traffic.
