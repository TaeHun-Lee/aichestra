...
If this prompt is being used through `manifest.adapter: command` or `manifest.adapter: llm`, return only a YAML document whose first non-whitespace text is `change_manifest:`. Do not include Markdown fences or prose. You may improve the generated draft, but every actual changed file from the adapter input must appear in structured fields such as `changed_areas[].file`, `newly_created_files`, or `deleted_or_renamed_files`. Aichestra will overwrite identity and diff evidence fields from local completion data after your response.
